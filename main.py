...
class Window:
    ...
    def __init__(self, window):
        ...
        self.lbl_pessoas=Label(window, text="Pessoas: ")
        self.lbl_pessoas.place(x=LEFT, y=50)
        self.combo = Combobox(window, values=self.cadastrados)
        self.combo.place(x=LEFT+50, y=50)
  

        self.lbl_nome=Label(window, text="Nome: ")
        self.lbl_nome.place(x=LEFT, y=100)
        self.txtfld=Entry(window, text="Nome", bd=1)
        self.txtfld.place(x=LEFT+50, y=100)

        self.adicionar_botoes(window)

    def adicionar_botoes(self, window):
        self.btn_add = Button(window, text="Adicionar", command=self.inserir)
        self.btn_add.place(x=LEFT, y=150)

        self.btn_remover = Button(window, text="Remover", command=self.remover)
        self.btn_remover.place(x=LEFT+100, y=150)

        self.btn_buscar = Button(window, text="Buscar", command=self.buscar)
        self.btn_buscar.place(x=LEFT+200, y=150)

        self.btn_limpar = Button(window, text="Limpar", command=self.limpar)
        self.btn_limpar.place(x=LEFT, y=200)

        self.btn_ordenar = Button(window, text="Ordenar", command=self.ordenar)
        self.btn_ordenar.place(x=LEFT+100, y=200)

        self.btn_tamanho = Button(window, text="Quantidade", command=self.mostrar_quantidade)
        self.btn_tamanho.place(x=LEFT+200, y=200)

        self.btn_maiusculo = Button(window, text="Converter Maiúsculo", command=self.deixar_maiusculo)
        self.btn_maiusculo.place(x=LEFT, y=250)

        self.btn_maiusculo = Button(window, text="Remover Repetidos", command=self.remover_repetidos)
        self.btn_maiusculo.place(x=LEFT+160, y=250)
